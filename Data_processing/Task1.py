...
number = input("Введите номер персонажа из журнала: ")

if number in characters:
    character = characters[number]
    for name, link in character.items():
        print("Имя персонажа:", name)
        print("Ссылка на персонажа:", list(link.keys())[0])
else:
    print("Персонажа с таким номером нет в журнале.")